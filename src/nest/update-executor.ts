...
export const UPDATE_EXECUTOR = Symbol('UPDATE_EXECUTOR');

/** Raised by `UpdateRunner` when no executor is registered. */
export class NoUpdateExecutorError extends Error {
  constructor() {
    super(
      'no UPDATE_EXECUTOR provided: the host must implement UpdateExecutorPort to apply updates',
    );
...
export class ReleaseManifestMissingError extends Error {
  constructor() {
    super(
      'the CMS returned no signed release manifest; refusing to apply an unverified update',
    );
